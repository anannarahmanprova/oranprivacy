	...
	for {
		...
		case ind, ok := <-stream.Indications():
			...
			case e2api.Encoding_PROTO:
				...
				decryptedPayload, e := decrypt(indMessageAsn1, encryptionKey)
				if e != nil {
					log.Warn("Decryption failed:", e)
					
				}
				log.Errorf("encryptedPayload........................................:", fmt.Sprintf("%x", indMessageAsn1))
				log.Errorf("decryptedPayload........................................:", fmt.Sprintf("%x", decryptedPayload))
				/*indMessageProto, err := serviceModelPlugin.IndicationMessageASN1toProto(decryptedPayload)
				if err != nil {

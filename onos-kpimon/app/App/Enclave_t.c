...
static sgx_status_t SGX_CDECL sgx_process_kpi(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_process_kpi_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_process_kpi_t* ms = SGX_CAST(ms_process_kpi_t*, pms);
	ms_process_kpi_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_process_kpi_t), ms, sizeof(ms_process_kpi_t))) {
...
SGX_EXTERNC const struct {
	...
	3,
	{
		{(void*)(uintptr_t)sgx_process_kpi, 0, 0},
